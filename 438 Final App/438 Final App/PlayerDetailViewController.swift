...
class PlayerDetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var ownedStocks: UILabel!
    @IBOutlet weak var sharesAvailable: UIView!
    ...
    var ownedStocksCount = 100
    var sharesAvailableCount = 1000
    ...
    override func viewDidLoad() {
        ...
        ownedStocks.text = String(ownedStocksCount)

    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.destination is BuySellViewController {
            let buySellVC = segue.destination as? BuySellViewController
            buySellVC?.shareNumber = ownedStocksCount
            buySellVC?.sharesOwnedCount = ownedStocksCount
            buySellVC?.sharesAvailableCount = sharesAvailableCount
        }
    }
